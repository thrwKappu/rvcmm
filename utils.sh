...
build_rv() {
	...
	for build_mode in "${build_mode_arr[@]}"; do
		...
		module_prop \
			...
			"RVCMM: ${app_name} - ${arch}" \
			"$version" \
			"${app_name} ${args[rv_brand]} v${version}. Original template by j-hc. Patched with ${rv_cli_jar#"$prebuilts_dir/"}, ${rv_integrations_apk#"$prebuilts_dir/"}, ${rv_patches_jar#"$prebuilts_dir/"}" \
