...
get_prebuilts() {
	...
	local rv_cli_url rv_integrations rv_integrations_changelog rv_integrations_url rv_patches rv_patches_changelog rv_patches_dl rv_patches_url
	...
	rv_integrations=$(gh_req "https://api.github.com/repos/${INTEGRATIONS_SRC}/releases/latest" -)
...
build_rv() {
	...
	for build_mode in "${build_mode_arr[@]}"; do
		...
		module_prop \
			...
			"RVCMM: ${app_name} - ${arch}" \
			"$version" \
			"${app_name} ${RV_BRAND} v${version}. Original template by j-hc. Patched with ${RV_CLI_JAR#"$PREBUILTS_DIR/"}, $INTEGRATIONS_SRC/${RV_INTEGRATIONS_APK#"$PREBUILTS_DIR/"}, $PATCHES_SRC/${RV_PATCHES_JAR#"$PREBUILTS_DIR/"}" \
