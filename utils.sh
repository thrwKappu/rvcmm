...
build_rv() {
	...
	for build_mode in "${build_mode_arr[@]}"; do
		...
		module_prop \
			...
			"RVCMM: ${app_name} - ${arch}" \
			"$version" \
			"${app_name} ${RV_BRAND} v${version}. Original template by j-hc. Patched with ${RV_CLI_JAR#"$PREBUILTS_DIR/"}, ${RV_INTEGRATIONS_APK#"$PREBUILTS_DIR/"}, ${RV_PATCHES_JAR#"$PREBUILTS_DIR/"}" \
