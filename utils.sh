	...
	for src_ver in "$cli_src CLI $cli_ver cli" "$patches_src Patches $patches_ver patches"; do
		...
		if [ "$tag" = "Patches" ]; then
			if [ "$grab_cl" = true ]; then
				echo -e "fetching changelog from $rv_rel"
...
--keystore-entry-password=123456789 --keystore-password=123456789 --signer=jhc --keystore-entry-alias=jhc -t '$patched_apk-tmp'  -t '$patched_apk-tmp' $patcher_args"
...
build_rv() {
	...
	if [ -n "$microg_patch" ] && [[ ${p_patcher_args[*]} =~ $microg_patch ]]; then
		wpr "MicroG related patches are automatically handled by builder."
	...
	for build_mode in "${build_mode_arr[@]}"; do
		...
		local brand_string="";
		if [[ ${rv_brand_f} != "revanced" ]]; then
			brand_string=" | ${args[rv_brand]} Edition"
		fi

		local patches_string="Selected Patch(es): ${args[included_patches]}, Excluded Patch(es): ${args[excluded_patches]}";
		if [ "${args[exclusive_patches]}" != true ]; then
			patches_string="Patches: ALL (Default)"
			if [[ -n ${args[included_patches]} ]]; then
				patches_string="${patches_string}, Extra: ${args[included_patches]}"
			fi
		...
		module_prop \
			...
			"RVCMM: $app_name - $arch$brand_string" \
			"$version" \
			"${patches_string} | Patched with: $(basename ${args[ptjar]}) on $(basename ${args[cli]%.jar})" \
