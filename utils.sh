...
get_rv_prebuilts() {
	...
	mkdir -p "$patches_dir" "$integrations_dir" "${TEMP_DIR}/rvcmm"
	...
	local rv_cli_jar="${TEMP_DIR}/rvcmm/${rv_cli_url##*/}"
	...
	rv_integrations=$(gh_req "$rv_integrations_rel" -)
	rv_integrations_changelog=$(json_get 'body' <<<"$rv_integrations" | sed 's/\(\\n\)\+/\\n/g')
	rv_integrations_url=$(json_get 'browser_download_url' <<<"$rv_integrations")
	...
	echo -e "${rv_integrations_changelog//# [/### [}\n---\n" >>"$patches_dir/changelog.md"
...
build_rv() {
	...
	for build_mode in "${build_mode_arr[@]}"; do
		...
		local module_output="RVCMM-${app_name_l}-v${version_f}-${arch_f}-${rv_brand_f}.zip"
...
description=${2}" >"${5}/module.prop"

	if [ "$ENABLE_MAGISK_UPDATE" = true ]; then echo "updateJson=${4}" >>"${5}/module.prop"; fi
}
