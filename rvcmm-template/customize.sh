...
if ! pmex path "$PKG_NAME" >&2; then
	if pmex install-existing "$PKG_NAME" >&2; then
		...
		else ui_print "  - Installed stock $PKG_NAME"; fi
	fi
fi
...
if BASEPATH=$(pmex path "$PKG_NAME"); then
  ...
  - This Module:    $PKG_VER
		...
		ui_print "  - $PKG_NAME is already up-to-date"
		INS=false
	fi
fi
		...
		if ! op=$(pmex install-commit "$SES"); then
			if echo "$op" | grep -q INSTALL_FAILED_VERSION_DOWNGRADE; then
				...
				if [ "$IS_SYS" = true ]; then
					...
					echo "mount /data/adb/rvcmm/empty $BASEPATH" >"$SCNM"
					chmod +x "$SCNM"
...
if [ "$KSU" ] && [ -d "/data/adb/modules/zygisk-assistant" ]; then
	ui_print "zygisk-assistant found, please grant root to $PKG_NAME if you have any problem"
fi
ui_print "  - Finished"